...
public class CoreDataOutput: DefaultOutput, CountdownDelegate {
    ...
    open override func display<TokenType, PayloadType>(_ message: Message<TokenType, PayloadType>) {
        let persistentMessage =  PersistentMessage(token: message.token.rawValue, tags: [], date: Date(), payload: "message.payload")
